...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate{
    ...
    @objc func timerFired() {
        ...
        timer_label.text = String(format: "Time Remaining: %.2f", seconds)
        
        // Stop the timer if it reaches zero
        if milliseconds == 0 {
            
            timer_label.textColor = UIColor.red
            timer?.invalidate()
    ...
    // UICollectionViewDataSource method to return how many cells are displayed
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        // Return number of cards
        return cardsArray.count
    }
    
    // UICollectionViewDataSource method to return cell that are displayed
